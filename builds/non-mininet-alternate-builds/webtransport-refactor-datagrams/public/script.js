...
transport.closed
    ...
    actual_latency = actual_latency/recvCount;
    console.log("result: " + actual_latency);
    perceived_latency = perceived_latency/recvFramesCount;
    console.log("result: " + perceived_latency);
    // console.log(sequenceNumbers);
    console.log("result: " + recvCount);
    })
    .catch(() => {
      ...
      switch (type) {
        case 'datagram':
            ...
            while (true) {
                ...
                sequenceNumber++;
                // if (!sequenceNumbers[sequenceNumber]) {
                //     sequenceNumbers[sequenceNumber] = 0;    // problem line come back to this it is resetting it to 0 every time it sends that is not good
                // }
...
async function readDatagrams(transport) {
    ...
    try {
      ...
      while (true) {
        ...
        // var { value, done } = await Promise.all([
        //     reader.read(),
        //     timeout(10000)
        // ]);
        if (done) {
        ...
        // console.log(sequenceNumbers)[sequenceNumber];
        // console.log("nice");
        // sequenceNumbers[sequenceNumber] = sequenceNumbers[sequenceNumber] + 1;

        // if (sequenceNumber == 1) {
        //     frame_first_sent = ts;
        // }

        actual_latency = actual_latency + ((Date.now()/1000) - ts);
        // if (recvCount == 1000) {
        //     actual_latency = actual_latency/1000;
        //     console.log(actual_latency);
        // }

        if (!dict[frameNumber]) {
          dict[frameNumber] = [];
          frameTimes[frameNumber] = ts;
        ...
        if (eof2 == 1) {
          ...
          render(connections[streamId2], frameNumber);
          delete dict[keys[idx]];
          prevSequenceNumber = 1;
        }
...
async function render(view, frameNumber){
    recvFramesCount++;
    perceived_latency = perceived_latency + ((Date.now()/1000) - frameTimes[frameNumber]);
