...
def changeLatency(node, new_latency):
    for intf in node.intfList(): # loop on interfaces of node
        if intf.link: # get link that connects to interface(if any)
            ...
            intfs[0].config(delay=new_latency)
            intfs[1].config(delay=new_latency)
...
def start():
    ...
    # bw_list = [100, 1]
    latency_list = ['0ms', '50ms']
    for latency in latency_list:
        net = Mininet(DumbellTopo(), link=TCLink, controller=DefaultController)
        nodes = net.switches + net.hosts
        net.start()
        for node in nodes:
            changeLatency(node, latency)

        f = open("results.csv", "a")
        writer = csv.writer(f)
        writer.writerow([str(latency)])
        f.close()

        perfTest(net, latency)


def perfTest(net, new_latency):
    ...
    filename = str(new_latency) + 'latency_capture_file.pcap'
    ...
    cli1.cmd('cd ../../../../../../home/mininet/Workspace/datagrams-test/tests/latency/streams')
    thread1 = cli1.popen('sudo python test-threading1-streams.py')
    time.sleep(5)
    thread2 = cli1.popen('sudo python test-threading2-streams.py')
